...
/// A rare multi-record input therefore panics on slice indexing.
pub fn parse_vulnerable(input: &[u8]) -> Result<Document, ParseError> {
    ...
    let records = ranges
        ...
        .map(|record| Record {
            tag: record.tag,
            // DVRA-003: stale range may be outside `normalized`.
...
/// Reference implementation: parses record boundaries first and normalizes each
