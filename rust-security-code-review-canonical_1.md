...
**Caveat: `forbid(unsafe_code)` protects against mistakes, not against a malicious author.** rustc has known, long-standing soundness holes (the I-unsound label; #25860 is open since 2015), and cve-rs demonstrates use-after-free, buffer overflow, and a working `transmute` in 100% safe Rust with `#![deny(unsafe_code)]` — it even carries its own advisory (RUSTSEC-2025-0028). Consequence for review: "the diff contains no `unsafe`" reduces the accident surface but is **not** a security boundary when reviewing untrusted contributions or dependencies — deliberate memory unsafety can be smuggled through safe code. Adversarial code review still requires reading the code, and Rust's type system must not be used as the sole isolation mechanism between trust domains.
...
12. **Higher-order invariants — never trust safe trait impls:** does the unsafe code assume a caller-provided trait behaves "reasonably"? `Iterator::size_hint` can lie, `ExactSizeIterator` can be wrong, `Read::read` can return an `n` larger than it wrote (CVE-2021-28875), `Borrow`/`Deref`/`Hash`/`Ord` can return different results on successive calls (CVE-2020-36323), any callback can panic mid-operation. Only `unsafe` traits (e.g. `TrustedLen`) carry contracts that unsafe code may rely on; every safe trait method must be treated as adversarial — both in what it returns and in whether it returns at all. This is Rudra's "higher-order invariant" pattern and the single largest class of stdlib soundness CVEs.
13. **Drop is not guaranteed:** does soundness depend on a destructor running? `mem::forget` is safe, `Rc` cycles leak — unsafe code must remain sound if `Drop` never executes. Guard-based designs must fail safe on leak, not just on unwind.
14. **Uninitialized memory exposure:** is an uninitialized buffer ever visible to safe code or caller-provided impls (e.g. passing an uninit `&mut [u8]` to a foreign `Read`), or copied into output? This is an information-disclosure class, not just UB (see the claxon memory-disclosure trophy). Use `MaybeUninit`/`BorrowedBuf` patterns; detect with MSan or libdiffuzz.
...
* **string slicing on non-UTF-8 char boundaries** — `&s[a..b]` panics if the byte offset falls inside a multi-byte character. The single most common panic class in the fuzz trophy case (found in clap, cookie, regex, juniper, bcrypt, and dozens more); any code that computes byte offsets into user-controlled text and slices with them is suspect — use `char_indices`, `get(a..b)`, or `is_char_boundary`;
...
* manual `unsafe impl Send` / `Sync` — `Send` and `Sync` are unsafe traits: other unsafe code is entitled to rely on their correctness. Each manual impl needs a full justification. The specific pattern to check: **generic impls with missing or wrong bounds** — `unsafe impl<T> Send for Wrapper<T>` without `T: Send`, or `T: Send` where the semantics actually require `T: Sync` (a `MutexGuard<Cell<i32>>` being `Sync` was CVE-2017-20004 in the stdlib itself). This is Rudra's third core pattern; verify the bound against what the wrapper actually shares across threads;
...
`transmute`; `unsafe impl Send/Sync` (especially generic, check the bounds); `mem::forget` outside `ManuallyDrop` patterns; `static mut`; `#[allow(...)]` without a justification comment; `as` casts in codecs; custom allocator or custom crypto; new `Ordering::Relaxed`; a guard held across `.await`; a hasher swap on attacker-controlled keys; unsafe code that calls back into caller-provided traits/closures while its invariants are broken; `reserve`/`set_len` driven by `size_hint` or any other safe trait method; an uninitialized buffer handed to a caller-provided `Read`/callback.
...
| stdlib CVE-2017-20004 (`MutexGuard<Cell<i32>>: Sync`) | Wrong bound on a generic `Sync` impl | Send/Sync generic bounds (§8); the stdlib itself got this wrong |
| stdlib CVE-2019-12083 (`Error::type_id`), CVE-2020-36323 (`join` for `[Borrow<str>]`), CVE-2021-28875 (`read_to_end`) | Unsafe code trusted an overridable safe trait method (identity, consistency, returned length) | Higher-order invariants (§2.3 item 12): safe trait impls are adversarial |
| stdlib CVE-2021-31162 (`Vec::from_iter`), CVE-2015-20001 (`BinaryHeap`), CVE-2020-36317 (`String::retain`) | Panic during element processing → double free / invalid state observable | Panic safety (§2.3 item 8): every callback and every `Drop` can panic |
| rustc #25860 / cve-rs / RUSTSEC-2025-0028 | UAF, buffer overflow, and `transmute` in 100% safe Rust via a compiler soundness hole | `forbid(unsafe_code)` is not an adversarial boundary (§2.2 caveat) |
...
**Training corpora for reviewer calibration and exercises.** No Rust equivalent of OWASP Juice Shop / DVWA exists yet — a single app with dozens of documented vulnerabilities. A strong program layers three tiers: (1) synthetic labeled CWE examples, (2) real RustSec CVEs reviewed blind against their fix commits, (3) an unlabeled small web service where nothing is marked in advance.

* **RustMizan** (arXiv 2607.04729, July 2026) — the closest thing to a labeled Rust vulnerability benchmark: *compilable* variants at crate/file/function level, annotated with presence, CWE class, vulnerable function, and exact lines, plus semantics-preserving mutants for contamination testing. Built to evaluate LLM agents, not train humans — but its headline result is the calibration argument for this whole document: frontier models in an agentic setup with CLI access hit only 56–65% on binary "vulnerable or not," line-level localization F1 stays near 20%, and adversarial cues drop line F1 by ~27%. Deciding a Rust diff is vulnerable is the *easy* part; pinpointing the defective lines is where both models and junior reviewers fail, which is exactly what §14 routing and mandatory unsafe sign-off exist to backstop. Good source for the "here is an unknown snippet — find it, class it, cite the lines, patch it, write a regression test" exercise format.
* **RustSec advisory DB + fix commits** — real production code with a known vulnerable range and usually a fix link. Mechanics: check out the pre-fix commit, run `cargo test` / `clippy --all-targets --all-features` / `+nightly miri test` / `cargo fuzz run <target>`, find the defect *without* reading the advisory, build a reproducer, assess impact, patch, add a regression test, then diff against the upstream fix. The TOSEM 2023 dataset (433 vulns with repos and fix commits) packages this at scale as a benchmark base.
* **Qwaz/rust-cve** — stdlib soundness CVEs, each a compact case study in one unsafe-review question from §2.3.
* **sslab-gatech/Rudra + Rudra-PoC** — reproducible PoCs for the ~264 bugs Rudra found; ready-made "why is this safe abstraction unsound?" exercises across the three Rudra patterns (panic safety, higher-order invariants, Send/Sync variance). More an unsafe-review lab than a web app.
* **rust-fuzz/trophy-case** — real fuzz findings categorized by class (`arith`, `oor`, `oom`, `so`, `loop`, `utf-8`, `uninit`, `logic`); useful both for picking harness targets and for knowing which panic classes dominate in practice. Note its convention: DoS/panics/OOM are *not* marked as security issues there — for a proxy/WAF handling adversarial traffic, they are.
* **cve-rs** — onboarding day one: what "the compiler guarantees it" does and does not mean.
* **Roll your own RustGoat** — for *web*-security training (authz, SSRF, path traversal, SQLi, JWT), where no mature Rust Juice Shop exists, a compact Axum/Actix app with `vulnerable/ fixed/ exploit/ tests/` per level beats anything off-the-shelf because it targets your actual stack. For a proxy/WAF team the highest-value custom levels are the ones public corpora don't cover: unwind-across-FFI, `as`-truncation of a wire length, guard-across-`.await`, a parser differential between two components, path-traversal with symlink/TOCTOU (fix is a generated `Uuid` filename, not just `canonicalize`). This doubles as an SME-AppSec-track test task.
...
**Training corpora (see §15):**

* RustMizan — https://arxiv.org/abs/2607.04729 · https://sfu-rsl.github.io/rust-mizan/ (compilable, multi-level, contamination-aware; LLM-agent benchmark)
* cve-rs — https://github.com/Speykious/cve-rs (memory unsafety in 100% safe Rust; RUSTSEC-2025-0028; rustc #25860)
* Qwaz/rust-cve — https://github.com/Qwaz/rust-cve (stdlib soundness CVE catalog)
* Rudra + PoCs — https://github.com/sslab-gatech/Rudra · https://github.com/sslab-gatech/Rudra-PoC
* rust-fuzz trophy case — https://github.com/rust-fuzz/trophy-case
* "A Closer Look at the Security Risks in the Rust Ecosystem" (TOSEM 2023, arXiv 2308.15046) — 433-vuln dataset with repos and fix commits, benchmark base
* Kobzol, "How memory safety CVEs differ between Rust and C/C++" (June 2026) — why unsoundness gets a CVE without a known exploit
